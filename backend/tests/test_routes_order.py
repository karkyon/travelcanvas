...
[Gate R0] /spots/test/ping・/travel-plans/test/pingはGate R0で
settings.DEBUG限定の診断用エンドポイントとなった。本ファイルはルート
順序の回帰テストが目的であり、実行環境の実際のDEBUG値に依存させない
ため、該当テストではsettings.DEBUGを明示的にTrueへ固定する。
"""
...
def test_spots_test_ping_not_swallowed_by_spot_id_route(client, monkeypatch):
...
def test_travel_plans_test_ping_not_swallowed_by_plan_id_route(client, monkeypatch):
    monkeypatch.setattr(settings, "DEBUG", True)
